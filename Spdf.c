...
int createParentDirectories(const char *dirPath) {
    ...
    // if the path starts with '/', move past the first /
    if (fullPath[0] == '/') {
        currentPos++;
    }
    ...
    while ((currentPos = strchr(currentPos, '/')) != NULL) {
        ...
        *currentPos = '/'; // restore the '/' and move to the next directory level
        currentPos++; // move past the current '/'
    }
...
int createTar() {
    // size_t command_size = strlen("tar -cf archive.tar -C ") + strlen(HOME_PATH) + strlen(" .") + 1;
    char *command = malloc(BUFFER_SIZE);
    snprintf(command, BUFFER_SIZE, "tar --exclude=*.tar -cf %s/pdf.tar -C %s .", HOME_PATH, HOME_PATH);
...
void sendDisplayToSocket(char* dirPath, int clientSocket){
    ...
    // the output of find command goes into this pipe
    if (pipe(pipefd) == -1) {
    ...
    if (pid == 0) {  // child : write output of find to pipe
        ...
        if (send(clientSocket, &totalBytes, sizeof(size_t), 0) == -1) {  
            printf("transfer error : failed to send metadata - display result size\n");
            free(output);
            exit(EXIT_FAILURE);
        }
        ...
        if (send(clientSocket, output, totalBytes, 0) == -1) {
            printf("transfer error : failed to send display results\n");
            free(output);
            exit(EXIT_FAILURE);
        }
...
void prcclient(int mainSocket){
    ...
    while(true){
        ...
        if(strcmp(tokens[0], UPLOAD) == 0){
            ...
            printf("info : exiting spdf client handler...\n");
            return;
        }
    }
}
...
int main(){
    ...
    serverAddress.sin_port = htons(PDF_PORT);                                           
